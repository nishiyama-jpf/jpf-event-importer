...
add_action( 'init', 'jpf_ensure_close_cron_scheduled' );
...
function jpf_ensure_close_cron_scheduled() {
    if ( wp_next_scheduled( JPF_CLOSE_CRON_HOOK ) ) {
        return;
    }

    wp_schedule_event( time() + MINUTE_IN_SECONDS, JPF_CLOSE_CRON_RECURRENCE, JPF_CLOSE_CRON_HOOK );
    jpf_add_close_log( 'info', '締切自動化Cronを再登録しました。', array() );
}
...
function jpf_run_track_usage_close_job() {
    ...
    if ( ! $trigger_timestamp || $current_timestamp < $trigger_timestamp ) {
        jpf_add_close_log( 'info', '自動削除をスキップ: 実行時刻前です。', array(
            'now'          => wp_date( 'Y-m-d H:i:s', $current_timestamp ),
...
function jpf_event_csv_importer_page() {
    ...
    if ( isset( $_POST['run_close_job_now'] ) && check_admin_referer( 'jpf_run_close_job_nonce' ) ) {
        jpf_run_track_usage_close_job();
        $message .= "<div class='updated'><p>締切自動化ジョブを手動実行しました。ログを確認してください。</p></div>";
    }
    ...
    <div class="wrap">
        ...
        <form method="post" style="margin-bottom: 12px;">
            <?php wp_nonce_field( 'jpf_run_close_job_nonce' ); ?>
            <?php submit_button( '締切自動化ジョブを今すぐ実行', 'primary', 'run_close_job_now', false ); ?>
        </form>
